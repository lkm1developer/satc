	...
	public function InitMasternode($coin_id,$credit,$tx_hash,$tx_index)
	{
		...
		//check this coins masternode setup in progress in middle not completed step 5
	 	$usermasternode=Usermasternode::
			where('masternode_id',$coin_id)->
			where('user_id',$user->id)->
			where('step','<',5)
			->count();
		//die;	
		if($usermasternode==0){
			 $ishashUsed=Usermasternode::where('masternode_id',$coin_id)
			...
			if($coin->py_name =='SMART'){
			...
			} else if($coin->py_name =='SATC'){
				$smart_serv=Srvr::where('satc', 1)->pluck('id')->toArray();
					if(is_array($smart_serv)):
					   $ip=IP::where('used',null)->whereIn('server_id',$smart_serv)->first();
				    endif;
			} 
			else if($coin->py_name =='XDNA'){
				$smart_serv=Srvr::where('xdna', 1)->pluck('id')->toArray();
					if(is_array($smart_serv)):
					   $ip=IP::where('used',null)->whereIn('server_id',$smart_serv)->first();
				    endif;
			} 
				...
				//$smart_serv=Srvr::where('smartcash', 0)->OrWhere('satc', null)->pluck('id')->toArray();
				$smart_serv=Srvr::where('others', 1)->pluck('id')->toArray();
				
				$ip=IP::where('used',null)->whereIn('server_id', $smart_serv)->first();
			//	echo '<pre>';
			//	var_dump($smart_serv);die;
			}
				//echo '<pre>';
				//var_dump($ip);
				//var_dump($smart_serv);die;
		...
		// return response()->json($ip);die();			
			$MN=Usermasternode::where('masternode_id',$coin_id)->where('user_id',$user->id)->count();
			//var_dump($MN);die();
			...
			$old_MN=$Usermasternode=Usermasternode::where('masternode_id',$coin_id)
			->where('user_id',$user->id)->where('step','<',5)->first();
		...
		if($coin->py_name =='SMART') {
			...
			if(is_array($smart_serv)):			
				$ip=IP::where('used',null)->whereIn('server_id',$smart_serv)->first();
						...
						//$smart_serv=Srvr::where('satc', 1)->first();
						$smart_serv=Srvr::where('satc', 1)->pluck('id')->toArray();
						if(is_array($smart_serv)):
					   $ip=IP::where('used',null)->whereIn('server_id',$smart_serv)->first();
				     endif;
					...
					$smart_serv=Srvr::where('xdna', 1)->pluck('id')->toArray();
						...
						$port->save();
						$ip->used=true;
						$ip->user_id=Auth::id();// ignor this column
						$ip->save();
